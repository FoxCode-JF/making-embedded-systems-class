...
extern volatile uint8_t nrf24_rx_flag, nrf24_tx_flag, nrf24_mr_flag, nrf24_interrupt_flag;
...
// Sets bits in status register
// input:
//	status - status to be written into register
void nrf24_SetStatus(uint8_t status)
{
	nRF24_WriteReg(nRF24_REG_STATUS, status);
}
